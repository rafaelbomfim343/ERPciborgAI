...
export function DatePickerWithRange({
  ...
  const firstAvailableDate = productionData.reduce(
    (minDate, current) => {
      const currentDate = parseISO(current.date);
      return currentDate < minDate ? currentDate : minDate;
    },
    parseISO(productionData[0].date),
  );

  const lastAvailableDate = productionData.reduce(
    (maxDate, current) => {
      const currentDate = parseISO(current.date);
      return currentDate > maxDate ? currentDate : maxDate;
    },
    parseISO(productionData[productionData.length - 1].date),
  );
